...
class Read_input(object):
	def __init__(self, path, image_dir, batch_size, num_epochs, shuffle = True, test_size = 10, distortion_range) :
	...
	def read_images_from_disk(self, images_list, labels_list, num_epochs, shuffle):
		...
		if tf.shape(example)[2] == 1:
			example = tf.image.rgb_to_grayscale(example)

		example = generate_random(example)
	...
	def read_image_labels(self, path, image_dir, batch_size, num_epochs, shuffle, test_size):
		...
		train_image, train_label = self.read_images_from_disk(images_list, labels_list, num_epochs, shuffle)

		image_batch, label_batch = tf.train.batch([train_image, train_label], batch_size=batch_size)

		test_image, test_label = self.read_images_from_disk(test_images_list, test_labels_list, 100000000, shuffle)
