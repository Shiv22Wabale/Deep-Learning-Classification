...
class CNN_CelebA(object) :
	...
	def define_cnn(self, x) :
		...
		h_pool1 = self.max_pool_2x2(h_conv1) # 128 * 128 * 8
		#--------1
		...
		h_pool2 = self.max_pool_2x2(h_conv2) # 64 * 64 * 16 
		#---------2
		...
		h_pool3 = self.max_pool_2x2(h_conv3) # 32 * 32 * 32 --- 
		#---------3

		#---------4 fc
		W_fc1 = self.weight_variable([32 * 32 * 32 * 4, 1024])
	...
	def train_model(self, path, image_dir, batch_size, num_epochs, shuffle, test_size):
		read_object = Read_input(path, image_dir, batch_size, num_epochs, shuffle, test_size)
			...
			try :
				while not coord.should_stop():
					...
					if i % 10 == 0:
						...
						summary, acc = sess.run([merge, accuracy], feed_dict={x: raw_images_test, y_: raw_labels_test, keep_prob: 1.0})
