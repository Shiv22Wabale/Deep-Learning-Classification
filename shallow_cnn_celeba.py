...
class CNN_CelebA(object) :
	...
	def define_cnn(self, x) :
		...
		h_conv1 = tf.nn.relu(self.conv2d_stride2(x_image, W_conv1) + b_conv1)
		h_pool1 = self.max_pool_3x3(h_conv1) # 112 * 112 * 64
		#--------1

		#---------2
		W_conv2 = self.weight_variable([1, 1, 64, 64])
		...
		#---------3
		W_conv3 = self.weight_variable([3, 3, 64, 192])
		b_conv3 = self.bias_variable([192])

		h_conv3 = tf.nn.relu(self.conv2d(h_conv2, W_conv3) + b_conv3)
		h_pool3 = self.max_pool_3x3(h_conv3) # 56 * 56 * 192 --- 
		#---------3

		#---------4 fc
		W_fc1 = self.weight_variable([56 * 56 * 48, 1024])
		b_fc1 = self.bias_variable([1024])

		h_pool3_flat = tf.reshape(h_pool3, [-1, 56 * 56 * 48])
	...
	def restore_model(self, sess) :
		self.saver.restore(sess, self.path_check_point)

	def test_model(self, path, image_dir, batch_size, num_epochs, shuffle, test_size, distortion_range):
		...
		with tf.Session() as sess:
			...
			self.restore_model(sess) # Restore the model tensorflow
