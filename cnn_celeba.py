...
class CNN_CelebA(object) :
	...
	def define_cnn(self, x) :
		...
		#--------1
		W_conv1 = self.weight_variable([5, 5, 3, 8])
		b_conv1 = self.bias_variable([8])

		h_conv1 = tf.nn.relu(self.conv2d(x_image, W_conv1) + b_conv1)
		h_pool1 = self.max_pool_2x2(h_conv1) # 32 * 32 * 8
		#--------1

		#---------2
		W_conv2 = self.weight_variable([5, 5, 8, 32])
		b_conv2 = self.bias_variable([32])

		h_conv2 = tf.nn.relu(self.conv2d(h_pool1, W_conv2) + b_conv2)
		h_pool2 = self.max_pool_2x2(h_conv2) # 16 * 16 * 32 
		#---------2

		#---------3
		W_conv3 = self.weight_variable([7, 7, 32, 64])
		b_conv3 = self.bias_variable([64])

		h_conv3 = tf.nn.relu(self.conv2d(h_pool2, W_conv3) + b_conv3)
		h_pool3 = self.max_pool_2x2(h_conv3) # 8 * 8 * 64 --- 
		#---------3

		#---------4 fc
		W_fc1 = self.weight_variable([8 * 8 * 64, 512])
